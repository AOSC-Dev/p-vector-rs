...
RETURNS text AS $$
  ...
  SELECT string_agg(translate(v[1] || '|',
    ...
    (CASE WHEN v[2]='' THEN '' ELSE public._comparable_digit(v[2]) END), '')
  FROM q1
...
RETURNS text AS $$
  SELECT ecmp || '!' || (CASE WHEN array_length(spl, 1)=1
    THEN public.comparable_ver(spl[1]) || '!1'
    ELSE public.comparable_ver(array_to_string(spl[1:array_length(spl, 1)-1], '-'))
    || '!' || public.comparable_ver(spl[array_length(spl, 1)]) END)
  FROM (
    SELECT (CASE WHEN epos=0 THEN '00'
      ELSE public._comparable_digit(substr(v, 1, epos-1)) END) ecmp, string_to_array(
